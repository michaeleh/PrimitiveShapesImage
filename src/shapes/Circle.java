...
public class Circle extends AbstractShape<Integer> {
    ...
    public void init(int width, int height, int factor) {
        ...
        this.maxWidth = width;
        this.scaleX.setValue(NumberUtils.randInt(MIN_IMAGE_COORDINATE, Math.min(width, height) / factor));
        this.scaleY.setValue(NumberUtils.randInt(MIN_IMAGE_COORDINATE, Math.min(width, height) / factor));
